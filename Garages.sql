...
CREATE TABLE socio (
    ...
    PRIMARY KEY (ID_persona),
    CONSTRAINT FK_socio_persona FOREIGN KEY (ID_persona) REFERENCES persona(ID_persona) ON DELETE CASCADE
);
...
DROP TRIGGER IF EXISTS tr_borrar_socio_persona $$
CREATE TRIGGER tr_borrar_socio_persona
AFTER DELETE ON persona
FOR EACH ROW
BEGIN
    -- Solo ejecutar si la persona borrada era un socio
    IF OLD.tipo_persona = 'socio' THEN

        -- 1. Limpiar GARAGES donde era propietario
        UPDATE garage g
        LEFT JOIN vehiculo v ON g.ID_vehiculoAsignado = v.ID_vehiculo
        SET 
            g.fechaCompra = NULL,
            g.ID_socioPropietario = NULL
        WHERE g.ID_socioPropietario = OLD.ID_persona;

        -- 2. Limpiar GARAGES donde era rentado
        UPDATE garage
        SET 
            fechaInicioRenta = NULL,
            fechaFinRenta = NULL,
            ID_socioRentado = NULL
        WHERE ID_socioRentado = OLD.ID_persona;

        -- 3. Borrar asignaciones de empleado relacionadas a los vehículos del socio
        DELETE ea
        FROM empleadoasignacion ea
        INNER JOIN vehiculo v ON ea.ID_vehiculoAsignado = v.ID_vehiculo
        WHERE v.ID_socioPropietario = OLD.ID_persona;

        -- 4. Borrar mantenimientos contratados relacionados a los vehículos del socio
        DELETE mc
        FROM mantenimientocontratado mc
        INNER JOIN vehiculo v ON mc.ID_vehiculo = v.ID_vehiculo
        WHERE v.ID_socioPropietario = OLD.ID_persona;

        -- 5. Borrar filas de socioasignacion que apunten a vehículos del socio
        DELETE sa
        FROM socioasignacion sa
        INNER JOIN vehiculo v ON sa.ID_vehiculoAsignado = v.ID_vehiculo
        WHERE v.ID_socioPropietario = OLD.ID_persona;

        -- 6. Borrar vehículos del socio
        DELETE v
        FROM vehiculo v
        WHERE v.ID_socioPropietario = OLD.ID_persona;

        -- 7. Borrar asignaciones directas del socio (si las hubiera)
        DELETE FROM socioasignacion
        WHERE ID_socio = OLD.ID_persona;
